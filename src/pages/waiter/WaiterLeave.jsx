...
function WaiterLeave() {
  ...
  const fetchLeaveRequests = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const response = await api.get('/employees/leave', {
  ...
  const handleSubmitLeave = async () => {
    try {
      setSubmitting(true);
      setSubmitError('');
      await api.post('/employees/leave', formData);
      setDialogOpen(false);
  ...
  const handleDeleteRequest = async () => {
    ...
    try {
      setDeleting(true);
      await api.delete(`/employees/leave/${selectedRequest.leaveId}`);
      ...
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
        <Box>
          ...
          <Typography color="text.secondary">
            {currentYear} entitlement: 14 annual · 7 casual · 7 medical days
          </Typography>
        </Box>
        ...
        <DialogContent>
          <Box sx={{ mt: 2, display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            {requestedDays > 0 && selectedBalance && (
              <Alert
                severity={requestedDays > selectedBalance.remaining ? 'error' : 'success'}
              >
